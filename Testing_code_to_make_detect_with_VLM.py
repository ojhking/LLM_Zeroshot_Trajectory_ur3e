...
class AgentState(TypedDict):
    ...
    refined_plan: Optional[List[str]]      
...
def task_classifier_agent(llm, state: AgentState, config=None) -> Dict[str, Any]:
    ...
    """
    system_prompt = ("""
You are a TaskClassifierAgent with strong semantic understanding.  
For each action phrase in the user’s command:
1. Identify the verb and object(s).
2. Explain in one sentence what the action means (e.g., "stack A on B" means "place A onto B").
3. If that meaning is “move an object from one place to another” (including synonyms like stack, lift, shift, slide, pile), 
   decompose it into the primitive sequence:  
     - grip <object>  
     - move to <destination>  
     - release <object>  
   and treat it as a **simple** action.
4. Otherwise, mark it as **unsupported** and extract the original verb+object phrase.
...
1. For each logical action phrase in the user input:
   a. Decide if this phrase involves moving an object X.
      - If yes **and** the gripper is not already holding X, emit “grip X” first.
   b. Then, if the phrase specifies a destination Y, emit “move to Y”.
   c. If the phrase is a placement (release X on Y), then after move emit “release X”.

2. To track whether the gripper is holding something, maintain an internal flag “holding”:
   - Initially false.
   - After “grip X”, set holding = X.
   - After “release X”, set holding = false.

3. This way, you only insert “grip X” when holding = false and an action needs it.

4. Finally output your plan as JSON:  
   `{"steps_planner": [...], "objects_to_detect": [...]}`  
...
# --- PlanRefinementAgent 정의 추가 ---
class RefineOutput(BaseModel):
    refined_plan: List[str]

def plan_refinement_agent(llm, state: AgentState, config=None) -> Dict[str, Any]:
    system_prompt = (
"""
You are PlanRefinementAgent.

Precondition:
- At startup, object_states map is fully initialized by hard-coded detections.

Rules:
1. Do NOT insert any detect before the first plan step.
2. Walk through each step in initial_plan in order:
   a. If the step begins with "release" (e.g. "release X on Y", "release X beside Y"):
      1. If it is NOT the final "release" in initial_plan, append exactly one
         "detect <object>"
         immediately after it, where <object> is the name of the object just released.
      2. Otherwise, append nothing.
   b. Otherwise, do NOT insert any detect.
3. **Post-processing**: If there is a `detect` step immediately following the final `release`, remove that detect.
4. Remove any duplicate detects.
5. Output exactly:
   {"refined_plan": [ ... ]}
...
"""
    ...
    # Prepare prompt with user input and initial_plan context
    messages = [
        SystemMessage(content=system_prompt),
        HumanMessage(content=json.dumps({
            "user_command": state['user_input'],
            "initial_plan": state['initial_plan']
        }))
    ]
    structured = llm.with_structured_output(RefineOutput)
    out = structured.invoke(messages, config=config)
    state['refined_plan'] = out.refined_plan
    msg = AIMessage(
        content=f"refined_plan: {out.refined_plan}",
        name="plan_refinement_agent"
    )
    return {"refined_plan": out.refined_plan, "messages": [msg]}
...
async def execution_sequence_planner_agent(llm, state: AgentState, config=None) -> Dict[str, Any]:
    """
    Detector 이후: refined_plan, 객체 위치 정보를 기반으로
    execution_sequence를 생성하고, 검증하고, 필요시 수정합니다.
    """

    def generate_simple_execution_sequence(state: AgentState) -> List[str]:
        """
        Detector 이후: refined_plan object 위치 정보를 기반으로 execution_sequence를 생성합니다.
        ...
        refined_plan = state["refined_plan"]
...
async def execution_runner_agent(state: AgentState, tool_node, config=None) -> Dict[str, Any]:
    ...
    for command in state["execution_sequence"]:
        ...
        if command.startswith("detect "):
            obj = command.split("detect ", 1)[1]
            print(f"[DEBUG] Running detection for {obj}")
            # 1) 호출
            response = await call_tool_and_wait(
                tool_node,
                AIMessage(content="", tool_calls=[{
                    "name": "find_object_3d_properties",
                    "args": {"object_name": obj},
                    "id": str(uuid4()),
                    "type": "tool_call"
                }]),
                config=config
            ...
            # 2) 파싱 & 업데이트
            new_loc = json.loads(response["messages"][0].content)
            state["location_of_object"][obj] = new_loc # 기존 위치값 업데이트
            print(f"[DEBUG] Detected {obj}: {new_loc}")
            continue
...
def create_total_graph(llm, tools):
    ...
    graph.add_node("PlanRefinementAgent", lambda state, config: plan_refinement_agent(llm, state, config))
    ...
    graph.add_conditional_edges(
        ...
        {"complex": END, "simple": "InitialPlannerAgent"}
    )
    graph.add_edge("InitialPlannerAgent", "PlanRefinementAgent")
    graph.add_edge("PlanRefinementAgent", "DetectorAgent")
